...
{
    public partial class OrganizersList : BasePage
    {
        ...
        {
            GetAuthorityUrl(this.Page, "Organizers/OrganizersList.aspx", 0);
        ...
        private void BindUsers()
        {
            /*
             * 暂时注释 2017.9.23
            OrganizeEO usersEO = GetOrgnizeUser(); // 获取登录用户的信息
            ...
            {
                strWhere.Append(string.Format(" And Organize.Ur_UserName like '%{0}%'", this.txtLoginName.Text.Trim()));
            ...
            {
                strWhere.Append(string.Format(" And Organize.Ur_Name like '%{0}%'", this.txtName.Text.Trim()));
            ...
            strWhere.Append(string.Format(" And Organize.Ur_state = '{0}'", this.rblState.SelectedValue));//0启用1停用2冻结 默认0
            ...
            if (leave < 200)
            {
                ...
                strWhere.Append(string.Format(" And cast(Organize.Ur_text1 as signed ) <= {0} ", Convert.ToInt32(usersEO.UrText1)));
            }

            if (leave < 90) // 根据用户账号管理权限显示归属内的账号
            {
                strWhere.Append(" And Organize.Ur_text2 like '%" + usersEO.UrId + ";%' ");
                //strWhere.Append("  And AUser.Ur_text2 like @URnname");
                //parm.Add(new MySqlParameter("@URnname", usersEO.UrId));
            }

            strWhere.Append(" And Organize.Ur_del = 0 ) ");
            ...
            if (!string.IsNullOrEmpty(this.ddlRoleName.SelectedValue)) // 角色
            {
                ...
                if (levelAccount>=100)
                {
                    strWhere.Append("  ) " + " and Arole.Rl_id = '" + this.ddlRoleName.SelectedValue + "' and  cast(Organize.Ur_text1 as signed ) <= 100 ");
                }
                else
                {
                    strWhere.Append("  ) " + " and Arole.Rl_id = '" + this.ddlRoleName.SelectedValue + "' and  cast(Organize.Ur_text1 as signed ) <= 90 ");
                }
            ...
            if (leave >= 80) // 显示自己的账号
            {
                strWhere.Append(" or Organize.Ur_id = '" + usersEO.UrId + "' ");
                //strWhere.Append("   or AUser.Ur_id =  @URID");
                //parm.Add(new MySqlParameter("@URID", usersEO.UrId));
            }
        ...
        {
            Response.Redirect("OrganizersEdit.aspx?State=A&Action=0");
        }
        ...
        protected void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (this.SelectCount() == 1)
            {
                ...
                Response.Redirect(string.Format("OrganizersEdit.aspx?State=U&Urid={0}&Action=1", strId));
            }
        ...
        protected void gvUsers_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowIndex > -1)
            {
                ...
                {
                    e.Row.Cells[12].Text = Resources.Resource.On;//"启用";
                }
                else if (e.Row.Cells[12].Text == "1")
                {
                    e.Row.Cells[12].Text = Resources.Resource.Off; //"停用";
                }
                else if (e.Row.Cells[12].Text == "2")
                {
                    e.Row.Cells[12].Text = "冻结";
                }
                else
                {
