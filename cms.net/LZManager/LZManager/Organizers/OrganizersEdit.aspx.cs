...
{
    public partial class OrganizersEdit : BasePage
    {
        ...
        private bool CheckUserLoginName()
        {
            ...
            strWhereC.Append("  Ur_UserName =@Ur_UserName AND Ur_del = 0 ");
            parmC.Add(new MySqlParameter("@Ur_UserName", this.txtLoginName.Text.Trim()));
            if (userManage.GetTable(strWhereC.ToString(),parmC).Rows.Count > 0)
            //if (userManage.GetTable(string.Format(" Ur_UserName = '{0}' AND Ur_del = 0", this.txtLoginName.Text.Trim())).Rows.Count > 0)
            {
                return false;//已存在
            }
            else
        ...
        private bool CheckForms()
        {
            ...
            else if (string.IsNullOrEmpty(this.txtOrganizeName.Text.Trim()))
            {
                flg = false;
                strAlert = Resources.Resource.Please_enter_user_name;// "请输入用户姓名";
                this.txtOrganizeName.Focus();
            }
